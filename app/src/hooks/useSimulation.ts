...
export function useSimulation(
  ...
  const runSimulation = useCallback(() => {
    ...
    const stepDuration = 2000; // 2 seconds per point
    const updateInterval = 100; // Update every 100ms for smooth animation
    const stepsPerPoint = stepDuration / updateInterval;
  ...
  }, [selectedPath, interpolate, calculateHeading, onPositionUpdate, clearSimulationInterval]);
  ...
  }, [selectedPath, clearSimulationInterval]);
