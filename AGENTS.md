...
- Hybrid location autocomplete (internal facilities + Photon API)
...
│   │   │   ├── LocationAutocomplete.tsx # Hybrid location search
...
│   │   │   ├── geocoding.ts     # Geocoding utilities (Photon API)
...
│   ├── docs/                    # Documentation
│   │   ├── AGENTS.md
│   │   ├── LOCATION_AUTOCOMPLETE_ARCHITECTURE.md
│   │   ├── TESTING_PLAN.md
│   │   └── TEST_RECOVERY_STATUS.md
...
```bash
# Development
npm run dev           # Start Next.js dev server with HMR (uses webpack)
...
**Note**: The Photon geocoding API requires no API key and works out of the box.
...
| `facilities` | Internal company facilities (hubs, ports, warehouses) |
...
Enabled on: `shipments`, `tracking_logs`, `notifications`, `facilities`
...
6. **Hybrid Search Pattern**: Internal facilities + external Photon API for locations

---

## User Roles

The system supports 8 user roles with different permissions:

| Role | Description |
|------|-------------|
| `super_admin` | Full platform access |
| `operations_manager` | Oversees all operations |
| `logistics_coordinator` | Creates and manages shipments |
...
| Photon API | Geocoding (no API key needed) | `src/lib/geocoding.ts` |
...
- `docs/TESTING_PLAN.md` - Original testing plan
- `docs/TEST_RECOVERY_STATUS.md` - Current test status
- `docs/LOCATION_AUTOCOMPLETE_ARCHITECTURE.md` - Location search architecture
- `supabase_schema.sql` - Database schema definition
...
7. **Environment**: This is a Next.js 16 + React 19 project with the App Router
8. **Styling**: Use Tailwind CSS with custom CSS variables for theming
